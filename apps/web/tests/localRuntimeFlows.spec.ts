...
const installBrowserState = async (
  page: Page,
  {
    ...
    aiMode?: "local_only" | "local_prefer";
  ...
  await page.addInitScript(
    ({ authKey, runtimeKey, token, injectToken }) => {
      const session = {
...
test.describe("browser-to-loopback local practice", () => {
  test("standard practice transcribes, evaluates, commits, and shows provenance", async ({
    ...
    const gatewayAuthorization = await installReadyGateway(page, "attempt-standard");
    let prepareBody: Record<string, unknown> | null = null;
    let commitBody: Record<string, unknown> | null = null;

    await page.route(`**/api/v1/tasks/${task.id}*`, (route) => json(route, task));
    ...
    await page.goto(`${baseURL ?? "http://localhost:5173"}/practice/${task.id}`);
    await dismissSetupWizard(page);
    await page.getByRole("button", { name: "Start recording" }).click();
    await page.getByRole("button", { name: "Stop recording" }).click();

    await expect(page.getByText("Local · unverified").first()).toBeVisible();
    await expect(page.getByText(/excluded from public rankings/i)).toBeVisible();
    expect(prepareBody).toEqual({ session_item_id: "session-item" });
  ...
  test("shared minigame runner binds the round and displays local provenance", async ({
    page,
    baseURL
  }) => {
    await installBrowserState(page);
    const gatewayAuthorization = await installReadyGateway(page, "attempt-minigame");
    let prepareBody: Record<string, unknown> | null = null;
    let finalized = false;
    ...
    await page.route(`**/api/v1/minigames/sessions/${sessionId}/state`, (route) =>
      json(route, {
        session: {
          id: sessionId,
          user_id: "user-1",
  ...
  test("pairs through Settings, persists across reload, and recovers after key rotation", async ({
    ...
    await page.route(`${gatewayOrigin}/**`, async (route) => {
      ...
      if (route.request().headers().authorization !== `Bearer ${acceptedPairingKey}`) {
    ...
    await expect(page.getByText("Pairing key saved in this browser.")).toBeVisible();
    ...
    await page.reload();
    await dismissSetupWizard(page);
    await expect(page.getByLabel("Local pairing key")).toHaveValue(rotatedPairingKey);
    await page.getByRole("button", { name: "Run all" }).click();
    await expect(page.getByText("Ready").first()).toBeVisible();
  });
  ...
  test("the existing deployed HTTPS origin can reach a real loopback gateway", async ({
    ...
    const result = await page.evaluate(async ({ origin, token }) => {
