...
export const LeaderboardPanel = ({
  ...
  const hasLocalScores = results.some(
    (result) => result.score_trust === "local_unverified",
  );
  const playerStats = players.map((player) => {
    const attempts = results.filter((result) => result.player_id === player.id);
    const average =
      attempts.length > 0
        ? attempts.reduce((sum, entry) => sum + entry.overall_score, 0) /
          attempts.length
        : 0;
    return {
      ...player,
  ...
  const teamSummary = teams
    .map((team) => {
      const members = playerStats.filter(
        (player) => player.team_id === team.id,
      );
      const average =
        members.length > 0
          ? members.reduce((sum, player) => sum + player.average, 0) /
            members.length
          : 0;
      return { ...team, average };
    })
      ...
      {mode === "tdm" && (
        <div className="mt-4 space-y-2">
          {teamSummary.map((team) => (
            ...
            >
              <span
                className={`rounded-full px-2 py-1 text-[10px] ${teamColorMap[team.color] ?? "bg-white/10 text-white"}`}
              >
                {team.name}
              </span>
      ...
      <div className="mt-4 space-y-2">
        {sortedPlayers.map((player, index) => {
          const team = teams.find(
            (teamEntry) => teamEntry.id === player.team_id,
          );
          return (
            <div
              key={player.id}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">
                    {t("minigameUi.roundCount", { count: player.rounds })}
                  </p>
                </div>
              </div>
