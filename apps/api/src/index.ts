...
import { z } from "zod";
import { createDbFromD1 } from "./db/d1";
import { exercises, attempts } from "./db/schema";
import { getRuntimeEnv, type RuntimeEnv, type RuntimeEnvBindings } from "./env";
...
export type ApiBindings = RuntimeEnvBindings & {
  DB: D1Database;
};

type ApiDatabase = DrizzleD1Database | BetterSQLite3Database;
...
type ApiAppOptions = {
  db?: ApiDatabase;
  createDb?: () => ApiDatabase;
  runtimeEnv?: RuntimeEnv;
};

export const createApiApp = (options: ApiAppOptions = {}) => {
  ...
  app.use(async (c, next) => {
    ...
    const runtimeEnv = options.runtimeEnv ?? getRuntimeEnv(c.env);
    const db = options.db ?? options.createDb?.() ?? createDbFromD1(c.env.DB);
  ...
  app.get("/api/v1/health/local-ai", async (c) => {
    ...
    const stt = await selectSttProvider("local_only", runtimeEnv).then(
      () => true,
      () => false
    );
    const llm = await selectLlmProvider("local_only", runtimeEnv).then(
      () => true,
      () => false
    );
  ...
  app.post("/api/v1/practice/run", async (c) => {
    ...
    const mode = input.mode ?? (runtimeEnv.aiMode as "local_prefer" | "openai_only" | "local_only");

    const sttProvider = await selectSttProvider(mode, runtimeEnv);
    const llmProvider = await selectLlmProvider(mode, runtimeEnv);
